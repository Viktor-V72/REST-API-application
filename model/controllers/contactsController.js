...
const changeContactController = async (req, res, next) => {
  ...
  try {
    const result = await changeContact(req.params.contactId, { name, email, phone }, req.user._id)
    if (!result) {
...
const updateStatusContactController = async (req, res, next) => {
  ...
  try {
    const result = await updateStatusContact(req.params.contactId, { favorite }, req.user._id)
    if (!result) {
